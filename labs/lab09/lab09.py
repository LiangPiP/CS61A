def insert_into_all(item, nested_list):
    ...
    for lst in nested_list:
        ...
        nested_list_new.append(lst_new)
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n==1 or n==2:
        ...
        count=0
        for i in range(1,n):
            count+=num_trees(n-i)*num_trees(i)
    return count
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock==0:
            ...
            self.balance=0
            self.stock-=1
            return f'Here is your candy and ${charge} change.'
...
def long_paths(t, n):
    ...
    def helper(t):
        if t.is_leaf():
            ...
            paths=[]
            for b in t.branches:
                paths+=[[t.label]+p for p in helper(b)]
            return paths
    return [path for path in helper(t) if len(path)>n]
...
def reverse_other(t):
    ...
    def reverse(t,flag):
        ...
        if flag:
            ...
            n=len(branches)
            for i in range(n//2):
                branches[i].label,branches[n-i-1].label=branches[n-i-1].label,branches[i].label
        for b in t.branches:
            reverse(b,1-flag)
    return reverse(t,1)
