...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    candidate=[para for para in paragraphs if select(para)]
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def selector(s):
        trim_s=split(remove_punctuation(lower(s)))
        for i in trim_s:
            if i in topic:
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    len1,len2=len(typed_words),len(source_words)
    if len1==0 and len2==0:
        return 100.0
    elif len1*len2==0:
        return 0.0
    match=[typed_words[i] for i in range(min(len1,len2)) if typed_words[i]==source_words[i]]
    return round(len(match)/len1*100,2)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    len_words=len(typed)
    return round(len_words*12/elapsed,2)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff=[diff_function(typed_word,s,limit) for s in word_list]
        diff_min=min(diff)

        if diff_min>limit:
            return typed_word
        else:        
            return word_list[diff.index(diff_min)]
...
def minimum_mewtations(start, goal, limit):
    ...
    if start==goal:  # Fill in the condition
        ...
        return 0
        # END
    if min(len(start),len(goal))==0:  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, times_per_player):
    ...
    for lists in times_per_player:
        times=times+[[lists[i+1]-lists[i] for i in range(len(lists)-1)]]
...
def fastest_words(match):
    ...
    for word in word_indices:
        ...
        for player in player_indices:
            if time(match, player, word)<time(match, fastest_player, word):
                fastest_player=player
        result[fastest_player].append(get_word(match, word))
    return result
