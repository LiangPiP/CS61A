...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.can_contain(self) and self.can_contain(place.ant)==False:
                place.ant.store_ant(self)
            elif self.can_contain(place.ant) and place.ant.can_contain(self)==False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.is_hive!=True:
            if place.bees!=[] and count>=self.lower_bound and count<=self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        ant_place,ant_health=self.place,self.health
        super().reduce_health(amount)
        bees=ant_place.bees[:]
        if self.health<=0:
            for bee in bees:
                bee.reduce_health(self.damage+ant_health)
        else:
            for bee in bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees!=[] and self.turns_to_chew==0:
            ...
            self.turns_to_chew=max(0,self.turns_to_chew-1)
# END Problem 7
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.num_queen==0:
            gamestate.num_queen=1
    ...
    def action(self, gamestate):
        ...
        while cur_place.exit!=None:
            cur_place=cur_place.exit
            ant=cur_place.ant
            if ant and isinstance(ant,ContainerAnt) and ant.ant_contained:
                Ant.double(ant)
                Ant.double(ant.ant_contained)
            elif ant and (not isinstance(ant,ContainerAnt) or (isinstance(ant,ContainerAnt) and not ant.ant_contained)):
                Ant.double(ant) 
            else:
                pass              
    ...
    def remove_from(self,gamestate):
        pass
