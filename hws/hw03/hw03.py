...
def flag(x):
    if x==1:
        return 1
    elif num_eights(x-1)>0 or (x-1)%8==0:
        return -flag(x-1)
    else:
        return flag(x-1)
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def constrant_count_coins(change,coin):
        if change==0:
            return 1
        elif change<0:
            return 0
            ...
            return constrant_count_coins(change-coin,coin)+constrant_count_coins(change,next_smaller_coin(coin))
    return constrant_count_coins(change,25)
