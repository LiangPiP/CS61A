...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        year_diff=Mint.present_year-self.year
        if year_diff>50:
...
def two_list(vals, counts):
    ...
    for i in range(len(counts)):
        val=vals[len(counts)-1-i]
        count=counts[len(counts)-1-i]
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value==0:
            ...
            next_fib=VirFib(self.value+self.prev)
...
def is_bst(t):
    ...
    if len(branches)==1:
        branch=branches[0]
        return is_bst(branch) and (bst_min(branch)<=t.label or bst_max(branch)>=t.label)
    elif len(branches)==2:
        left,right=branches
